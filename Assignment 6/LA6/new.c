...
int main()
{
    ...
    while (!emptyQ(E))
    {
        ...
        if (e.type == 'P')
        {
            ...
            if (patient_count > 25)
            {
                printf("[%02d:%02d %s] Patient %d leaves (25 patients already arrived)\n", hours, minutes, am ? "AM" : "PM", patient_count);
                pthread_mutex_unlock(&pmutex);
                continue;
            }
        ...
        if (e.type == 'S')
        {
            ...
            if (done)
            {
                printf("[%02d:%02d %s] Salesrep %d leaves (session over)\n", hours, minutes, am ? "AM" : "PM", salesrep_count);
                pthread_mutex_unlock(&smutex);
                continue;
            }
            ...
            if (salesrep_count > 3)
            {
                printf("[%02d:%02d %s] Salesrep %d leaves (3 salesreps already arrived)\n", hours, minutes, am ? "AM" : "PM", salesrep_count);
                pthread_mutex_unlock(&smutex);
                continue;
            }
        ...
        if ((patient_count == 25 && salesrep_count > 3) || (patient_count > 25 && salesrep_count == 3))
        {
            // make done flag 1
            // signal doctor
            pthread_mutex_lock(&dmutex);
